...
add alliance_party varchar(150)

--INDIA Alliance
UPDATE partywise_results
SET alliance_party = 'I.N.D.I.A'
WHERE party IN (
    'Indian National Congress - INC',
    'Aam Aadmi Party - AAAP',
    'All India Trinamool Congress - AITC',
...
--Other
UPDATE partywise_results
SET alliance_party = 'OTHER'
WHERE alliance_party IS NULL;

select alliance_party,sum(won)
from partywise_results
group by alliance_party

--8) Winning candidate's name, their party name, total votes, and the margin of victory for a specific state and constituency?
SELECT cr.Winning_Candidate, p.Party, p.alliance_party, cr.Total_Votes, cr.Margin, cr.Constituency_Name, s.State
FROM constituencywise_results cr
JOIN partywise_results p ON cr.Party_ID = p.Party_ID
JOIN statewise_results sr ON cr.Parliament_Constituency = sr.Parliament_Constituency
JOIN states s ON sr.State_ID = s.State_ID
WHERE s.State = 'Karnataka' AND cr.Constituency_Name = 'UDUPICHIKMAGALUR';
...
WHERE 
    cr.Constituency_Name = 'UDUPICHIKMAGALUR'
...
sum(case when
		p.alliance_party = 'NDA'
		  then 1
		    else 0
			 end) as NDA_Seats_Won,
sum(case when
		p.alliance_party = 'I.N.D.I.A' 
		  then 1
		    else 0
			 end) as INDIA_Seats_Won,
sum(case when
		p.alliance_party = 'OTHER' 
		  then 1
		    else 0
...
join states s on sr.State_ID = s.State_ID
where
p.alliance_party in ('NDA', 'I.N.D.I.A',  'OTHER')
group by s.State
order by s.State;
...
--13) Which candidate won and which candidate was the runner-up in each constituency of State for the 2024 elections?
	...
	-- Step 1: Create a list of candidates with their vote rank in each constituency
...
--15) Which party alliance (NDA, I.N.D.I.A, or OTHER) won the most seats across all states?
SELECT 
    p.alliance_party,
